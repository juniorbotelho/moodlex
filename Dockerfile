...
ADD --chown=root:root ${GITHUB_RAW}/config/fastcgi.conf "/etc/nginx/fastcgi.conf"
ADD --chown=root:root ${GITHUB_RAW}/config/nginx.conf "/etc/nginx/http.d/moodle.conf"
...
RUN chown -R root:root "/var/www/html/moodle" &&\
    ...
    chmod -R 0755 "/var/www/html/moodledata" &&\
    # Configure scripts that will be used to enable extensions and much more
